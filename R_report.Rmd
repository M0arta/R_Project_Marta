...
:::
-   As it is visible from the histogram around 2016 there has been a **significant increase in the internet access** preceded by a steady increasing trend across all the previous years, and the literature reports this spike to be related mainly to a more diffused access to mobile internet. At the same time Zambia has hold two main elections in the period around the increase in the mobile connection bandwidth: one in 2011 and one in 2016.
...
```{r anotheryearr, echo=FALSE, message=FALSE, warning=FALSE}
knitr::kable(head(mobile_2011), format = "markdown", booktabs = TRUE, longtable = TRUE)
...
```{r yearr, echo=FALSE, message=FALSE, warning=FALSE}
knitr::kable(head(mobile_2016), format = "latex", booktabs = TRUE, longtable = TRUE)
```
...
knitr::kable(head(joined_sf), format = "markdown", booktabs = TRUE, longtable = TRUE)
```
